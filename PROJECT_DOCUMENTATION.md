...
```text
db/20260707_cat_dog_standardization.sql
```

执行该脚本后，`category` 被收敛为一级分类“猫/狗”，新增 `breed` 品种字典表，并为 `animals` 增加 `category_id`、`breed_id`。`species` 字段暂时保留兼容旧数据和旧接口。
...
| 方法 | 路径 | 说明 | 后端权限 |
|---|---|---|---|
| `GET` | `/api/animal/list` | 分页查询动物，支持名称、种类、审核状态、领养状态筛选 | 匿名放行 |
